...
public class EciesSect571k1Executor extends EciesExecutor {
    ...
    public String encrypt(EncryptAndDecryptOptions options, String plaintext) throws EncryptionException {
        return encrypt(options.getBase64RemotePublicKey(), plaintext, CurveType.SECT571K1);
    }

    @Override
    public String decrypt(EncryptAndDecryptOptions options, String base64Ciphertext) throws EncryptionException {
        return decrypt(options.getBase64HostedPrivateKey(), base64Ciphertext, CurveType.SECT571K1);
    }
}
