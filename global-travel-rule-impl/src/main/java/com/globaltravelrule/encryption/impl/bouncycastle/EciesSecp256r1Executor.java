...
public class EciesSecp256r1Executor extends EciesExecutor {
    ...
    public String encrypt(EncryptAndDecryptOptions options, String plaintext) throws EncryptionException {
        return encrypt(options.getBase64RemotePublicKey(), plaintext, CurveType.SECP256R1);
    }

    @Override
    public String decrypt(EncryptAndDecryptOptions options, String base64Ciphertext) throws EncryptionException {
        return decrypt(options.getBase64HostedPrivateKey(), base64Ciphertext, CurveType.SECP256R1);
    }
}
